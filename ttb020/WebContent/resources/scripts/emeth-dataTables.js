...
var dataTableEMETH = function() {
	 ...
	 * @returns Objeto DataTable creado
	 */
	function createNoResponsiveDataTableWhitOutInfo(idTable, aoColumnDefs) {
		return $("#" + idTable).dataTable({
			language : {
				url : $("#hdnRutaContexto").val() + getUrlLanguage()
			},
	 ...
	 * ordenamiento
	 * 
	 * EJEMPLO: dataTableEMETH.createSortedDataTable("tblFases",1,"asc",0,"asc", NUM_COLUM, "asc|desc",...);
	 * 
	 * @param idTable
	 *            {String} identificador del elemento table a ser creado como
	 *            objeto DataTable
	...
	function createSortedDataTable() {
		...
		var columnas = [((arguments.length) - 1) / 2];
	...
	return {
		...
		createNoResponsiveDataTableWhitOutInfo : createNoResponsiveDataTableWhitOutInfo,
